...
var CoreImpl = {
    ...
    parseDOM: function (node) {
        ...
        for (var i = 0; i < node.children.length; i++) {
            ...
            if (child)
                CoreImpl.parseDOM(child);
        }
    },
    create: function (type) {
        type = type.toLowerCase();
        if (CoreImpl.cache[type]) {
            var component = CoreImpl.cache[type];
            return new component.constructor();
        }
        if (CoreImpl.components) {
            if (CoreImpl.components[type] && CoreImpl.components[type].length > 0) {
                for (var i = 1; i < CoreImpl.components[type].length; i++) {
                    var component = CoreImpl.components[type][i];
    ...
    setup: function () {
        ...
        document["parseDOM"] = function (node) { return CoreImpl.parseDOM(node); };
        window["Core"] = function (type) { return Core(type); };
        CoreImpl.parseDOM(document.head);
        if (!CoreImpl.Style) {
            var obj = CoreImpl.createObject("bao/style", null, "style");
            obj.removeAttribute("style");
            var child = document.head.firstChild;
            if (child)
                document.head.insertBefore(obj, child);
            else
        ...
        var reparse = false;
        if (!CoreImpl.Focus) {
            var obj = CoreImpl.createObject("bao/focusManager");
            document.body.appendChild(obj);
            reparse = true;
        }
        if (!CoreImpl.DataStore) {
            var obj = CoreImpl.createObject("bao/dataStore");
            document.body.appendChild(obj);
            reparse = true;
        }
        if (!CoreImpl.MetaConfig.element) {
            var obj = CoreImpl.MetaConfig.$createElement("object");
    ...
    createObject: function (id, type, tag) {
        if (!type)
            type = id;
        if (!tag)
            tag = "object";
...
function Core(type) {
    if (type)
        return CoreImpl.create(type);
    else
        return CoreImpl;
}
exports["default"] = Core;
window.addEventListener("load", function () { return CoreImpl.setup(); });
